...
public class UDPClient {
    ...
    public void sendRequest(Request request) {
		try {
			Utils.logMessage("Request as String: " + request.getMessage()+" to be sent to port "+request.getSenderPort()+" and on IPaddress "+request.getSenderIpAddress());
			
			byte []buffer = request.getMessage().getBytes(defaultCharset);
			DatagramPacket p = new DatagramPacket(buffer, buffer.length, request.getSenderIpAddress(), request.getSenderPort());
    ...
    public Response receiveResponse() {
		...
		try {
			...
			response.setReceiverIpAddress(p.getAddress());
			response.setReceiverPort(p.getPort());
        } catch (IOException ex) {
        	ex.printStackTrace();
        }
